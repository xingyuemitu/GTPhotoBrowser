...
@interface GTPreviewCell ()

@end

@implementation GTPreviewCell
...
- (GTPreviewView *)previewView
{
    if (!_previewView) {
        _previewView = [[GTPreviewView alloc] initWithFrame:self.bounds];
...
- (void)layoutSubviews
{
    ...
    if (self.model.type == GTAssetMediaTypeImage ||
        self.model.type == GTAssetMediaTypeGif ||
        (self.model.type == GTAssetMediaTypeLivePhoto && !self.showLivePhoto) ||
        self.model.type == GTAssetMediaTypeNetImage) {
        self.imageGifView.frame = self.bounds;
    } else if (self.model.type == GTAssetMediaTypeLivePhoto) {
        self.livePhotoView.frame = self.bounds;
    } else if (self.model.type == GTAssetMediaTypeVideo) {
        self.videoView.frame = self.bounds;
    } else if (self.model.type == GTAssetMediaTypeNetVideo) {
        self.netVideoView.frame = self.bounds;
    }
}

- (GTPreviewImageAndGif *)imageGifView
{
    if (!_imageGifView) {
        _imageGifView = [[GTPreviewImageAndGif alloc] initWithFrame:self.bounds];
...
- (GTPreviewLivePhoto *)livePhotoView
{
    if (!_livePhotoView) {
        _livePhotoView = [[GTPreviewLivePhoto alloc] initWithFrame:self.bounds];
        _livePhotoView.singleTapCallBack = self.singleTapCallBack;
    }
    return _livePhotoView;
}

- (GTPreviewVideo *)videoView
{
    if (!_videoView) {
        _videoView = [[GTPreviewVideo alloc] initWithFrame:self.bounds];
        _videoView.singleTapCallBack = self.singleTapCallBack;
    }
    return _videoView;
}

- (GTPreviewNetVideo *)netVideoView
{
    if (!_netVideoView) {
        _netVideoView = [[GTPreviewNetVideo alloc] initWithFrame:self.bounds];
...
- (void)setModel:(GTPhotoModel *)model
{
    ...
    switch (model.type) {
        ...
        case GTAssetMediaTypeVideo: {
            ...
            break;
        case GTAssetMediaTypeNetImage: {
            [self addSubview:self.imageGifView];
            [self.imageGifView loadImage:model.image?:model.url];
        }
            break;
        case GTAssetMediaTypeNetVideo: {
...
- (void)reload
{
    if (self.showGif &&
        ...
        [self.imageGifView loadGifImage:self.model.asset];
               ...
               self.model.type == GTAssetMediaTypeLivePhoto) {
...
- (void)resumePlay
{
    if (self.model.type == GTAssetMediaTypeGif) {
...
//!!!!: GTBasePreviewView
@implementation GTBasePreviewView
...
//!!!!: GTPreviewImageAndGif
@interface GTPreviewImageAndGif () <UIScrollViewDelegate>

@end

@implementation GTPreviewImageAndGif
...
- (void)loadGifImage:(PHAsset *)asset
{
    ...
    [GTPhotoManager requestOriginalImageDataForAsset:asset completion:^(NSData *data, NSDictionary *info) {
        gt_strongify(weakSelf);
        if (![[info objectForKey:PHImageResultIsDegradedKey] boolValue]) {
            strongSelf.imageView.image = [GTPhotoManager transformToGifImageWithData:data];
...
- (void)loadNormalImage:(PHAsset *)asset
{
    ...
    gt_weakify(self);
    self.imageRequestID = [GTPhotoManager requestImageForAsset:asset size:size completion:^(UIImage *image, NSDictionary *info) {
        gt_strongify(weakSelf);
...
- (void)loadImage:(id)obj
{
    ...
    if ([obj isKindOfClass:UIImage.class]) {
        ...
        [self.imageView sd_setImageWithURL:obj placeholderImage:nil options:SDWebImageProgressiveDownload completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            ...
            [strongSelf.indicator stopAnimating];
            if (error) {
                ShowToastLong(@"%@", GetLocalLanguageTextValue(GTPhotoBrowserLoadNetImageFailed));
            } else {
                strongSelf.loadOK = YES;
...
//!!!!: GTPreviewLivePhoto
@implementation GTPreviewLivePhoto
...
{
    gt_weakify(self);
    [GTPhotoManager requestLivePhotoForAsset:asset completion:^(PHLivePhoto *lv, NSDictionary *info) {
        gt_strongify(weakSelf);
        if (lv) {
...
- (UIButton *)playBtn
{
    if (!_playBtn) {
        ...
        [_playBtn setBackgroundImage:GetImageWithName(@"gt_playVideo") forState:UIControlStateNormal];
...
- (UILabel *)icloudLoadFailedLabel
{
    if (!_icloudLoadFailedLabel) {
        ...
        attach.image = GetImageWithName(@"gt_videoLoadFailed");
        ...
        NSAttributedString *lastStr = [[NSAttributedString alloc] initWithString:[NSBundle gtLocalizedStringForKey:GTPhotoBrowseriCloudVideoText]];
...
- (void)loadNormalImage:(PHAsset *)asset
{
    ...
    if (![GTPhotoManager judgeAssetisInLocalAblum:asset]) {
...
- (void)singleTapAction
{
    ...
    if (!_playLayer) {
        gt_weakify(self);
        [GTPhotoManager requestVideoForAsset:self.asset completion:^(AVPlayerItem *item, NSDictionary *info) {
...
//!!!!: GTPreviewNetVideo
@implementation GTPreviewNetVideo
{
