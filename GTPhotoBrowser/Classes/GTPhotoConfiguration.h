...
@interface GTPhotoConfiguration : NSObject
...
/**
 ...
 @discussion e.g.:1:1，请使用GTPhotoDefine中所提供方法 GetClipRatio(NSInteger value1, NSInteger value2)，该数组可不设置，有默认比例，为（Custom, 1:1, 4:3, 3:2, 16:9），如果所设置比例只有一个且 为 Custom 或 1:1，则编辑图片界面隐藏下方比例工具条
 */
...
/**
 ...
 @example: 开发者需要替换选中与未选中的图片资源，则需要传入的数组为 @[@"gt_btn_selected", @"gt_btn_unselected"]，则框架内会使用开发者项目中的图片资源，而其他图片则用框架bundle中的资源
 */
...
/**
 ...
 @discussion 如果选择了大量图片，框架一下解析大量图片会耗费一些内存，开发者此时可置为NO，拿到assets数组后使用 GTPhotoManager 中提供的 "anialysisAssets:original:completion:" 方法进行逐个解析，以达到缓解内存瞬间暴涨的效果，该值为NO时，回调的图片数组为nil
 */
@property (nonatomic, assign) BOOL shouldAnialysisAsset;

/**
 框架语言，默认 GTLanguageSystem (跟随系统语言)
 */
@property (nonatomic, assign) GTLanguageType languageType;

/**
 支持开发者自定义多语言提示，但是所自定义多语言的key必须与原key一致
 @example: 开发者需要替换 key: "GTPhotoBrowserLoadingText"，value:"正在处理..." 的多语言，则需要传入的字典为 @{@"GTPhotoBrowserLoadingText": @"需要替换的文字"}，而其他多语言则用框架中的（更改时请注意多语言中包含的占位符，如%ld、%@）
 */
...
/**
 视频清晰度，默认GTCaptureSessionPreset1280x720
 */
@property (nonatomic, assign) GTCaptureSessionPreset sessionPreset;

/**
 录制视频及编辑视频时候的视频导出格式，默认GTExportVideoTypeMov
 */
@property (nonatomic, assign) GTExportVideoType exportVideoType;
