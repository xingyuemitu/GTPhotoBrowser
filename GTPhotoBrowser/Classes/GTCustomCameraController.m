...
- (void)setupUI
{
    ...
    self.dismissBtn.frame = CGRectMake(60, self.bounds.size.height/2-25/2, 25, 25);
    [self.dismissBtn setImage:GetImageWithName(@"gt_arrow_down") forState:UIControlStateNormal];
    ...
    [self.cancelBtn setImage:GetImageWithName(@"gt_retake") forState:UIControlStateNormal];
    ...
    [self.doneBtn setImage:GetImageWithName(@"gt_takeok") forState:UIControlStateNormal];
...
//--------------------------------------------------------//
//--------------------------------------------------------//
@interface GTCustomCameraController () <CameraToolViewDelegate, AVCaptureFileOutputRecordingDelegate>
{
    //拖拽手势开始的录制
    BOOL _dragStart;
    BOOL _layoutOK;
...
//播放视频
@property (nonatomic, strong) GTPlayer *playerView;
...
@implementation GTCustomCameraController
...
- (void)viewDidLayoutSubviews
{
    ...
    self.toolView.frame = CGRectMake(0, kViewHeight-130-GT_SafeAreaBottom, kViewWidth, 100);
...
- (void)setupUI
{
    ...
    self.focusCursorImageView = [[UIImageView alloc] initWithImage:GetImageWithName(@"gt_focus")];
    ...
    [self.toggleCameraBtn setImage:GetImageWithName(@"gt_toggle_camera") forState:UIControlStateNormal];
...
- (NSString *)transformSessionPreset
{
    switch (self.sessionPreset) {
        case GTCaptureSessionPreset325x288:
            return AVCaptureSessionPreset352x288;
            
        case GTCaptureSessionPreset640x480:
            return AVCaptureSessionPreset640x480;
            
        case GTCaptureSessionPreset1280x720:
            return AVCaptureSessionPreset1280x720;
            
        case GTCaptureSessionPreset1920x1080:
            return AVCaptureSessionPreset1920x1080;
            
        case GTCaptureSessionPreset3840x2160:
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    if (point.y > [UIScreen mainScreen].bounds.size.height-150-GT_SafeAreaBottom) {
        return;
    }
...
- (void)onStartRecord
{
    ...
    if (![self.movieFileOutPut isRecording]) {
        NSURL *url = [NSURL fileURLWithPath:[GTPhotoManager getVideoExportFilePath:self.videoType]];
...
{
    if (!_playerView) {
        self.playerView = [[GTPlayer alloc] initWithFrame:self.view.bounds];
