...
@interface GTImageEditTool ()
{
    NSInteger _layoutCount;
    
    GTImageEditType _type;
    GTPhotoConfiguration *_configuration;
    GTImageEditType _selectToolType;
    ...
    GTClippingCircle *_ltView;
    GTClippingCircle *_lbView;
    GTClippingCircle *_rtView;
    GTClippingCircle *_rbView;
    
    CGSize _originSize;
}

@property (nonatomic, strong) GTBrushBoardImageView *imageView;
...
@property (nonatomic, strong) GTGridLayar *gridLayer;

@property (nonatomic, strong) GTClipItem *selectClipItem;
@property (nonatomic, strong) GTClipRatio *selectClipRatio;
@property (nonatomic, assign) CGRect clippingRect;

@property (nonatomic, strong) GTFilterItem *selectFilterItem;
@property (nonatomic, strong) GTDrawItem *selectDrawItem;

@end

@implementation GTImageEditTool
...
{
    return [self initWithEditType:GTImageEditTypeDraw |
            GTImageEditTypeMosaic |
            GTImageEditTypeFilter |
            GTImageEditTypeClip |
            GTImageEditTypeRotate
                            image:nil
                    configuration:nil];
}
...
- (instancetype)initWithEditType:(GTImageEditType)type image:(UIImage *)image configuration:(GTPhotoConfiguration *)configuration
{
...
- (void)layoutSubviews
{
    ...
    self.cancelBtn.frame = CGRectMake(15+inset.left, inset.top+10, GetMatchValue(GetLocalLanguageTextValue(GTPhotoBrowserCancelText), 15, YES, 30), 30);
    CGFloat doneBtnW = GetMatchValue(GetLocalLanguageTextValue(GTPhotoBrowserDoneText), 15, YES, 30);
    self.doneBtn.frame = CGRectMake(kViewWidth-doneBtnW-inset.right-15, inset.top+10, doneBtnW, 30);
    
    [self setImageViewFrame:_layoutCount != 1];
    if (_selectToolType & GTImageEditTypeClip) {
...
- (void)setImageViewFrame:(BOOL)animate
{
    ...
    } else if (_selectToolType & (GTImageEditTypeFilter | GTImageEditTypeDraw)) {
        showMenu =  YES;
    }
...
- (void)setupUI
{
    ...
    [self.cancelBtn setTitle:GetLocalLanguageTextValue(GTPhotoBrowserCancelText) forState:UIControlStateNormal];
    ...
    [self.doneBtn setTitle:GetLocalLanguageTextValue(GTPhotoBrowserDoneText) forState:UIControlStateNormal];
    ...
    //imageView
    self.imageView = [[GTBrushBoardImageView alloc] init];
    ...
    
    {
        if (_type & GTImageEditTypeClip) {
            ...
            [self.clipBtn setImage:GetImageWithName(@"gt_clip") forState:UIControlStateNormal];
        ...
        if (_type & GTImageEditTypeRotate) {
            ...
            [self.rotateBtn setImage:GetImageWithName(@"gt_rotateimage") forState:UIControlStateNormal];
        ...
        if (_type & GTImageEditTypeFilter) {
            ...
            [self.filterBtn setImage:GetImageWithName(@"gt_filter") forState:UIControlStateNormal];
        ...
        if (_type & GTImageEditTypeDraw) {
            ...
            [self.drawBtn setImage:GetImageWithName(@"gt_draw") forState:UIControlStateNormal];
...
- (GTClippingCircle*)clippingCircleWithTag:(NSInteger)tag
{
    GTClippingCircle *view = [[GTClippingCircle alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
...
- (NSInteger)toolCount
{
    NSArray *arr = @[@(GTImageEditTypeClip), @(GTImageEditTypeRotate), @(GTImageEditTypeFilter), @(GTImageEditTypeDraw), @(GTImageEditTypeMosaic)];
    NSInteger count = 0;
...
- (void)setupFilterMenu
{
    ...
    for (int i = GTFilterTypeOriginal; i <= GTFilterTypeColorInvert; i++) {
        GTFilterItem *item = [[GTFilterItem alloc] initWithFrame:CGRectMake(x, 0, W, H) Image:iconImage filterType:i target:self action:@selector(tapFilter:)];
...
- (void)setupDrawMenu
{
    ...
    [self.revokeBtn setImage:GetImageWithName(@"gt_revoke") forState:UIControlStateNormal];
    ...
    for (int i = GTDrawItemColorTypeWhite; i <= GTDrawItemColorTypePurple; i++) {
        GTDrawItem *item = [[GTDrawItem alloc] initWithFrame:CGRectMake(x, 0, W, H) colorType:i target:self action:@selector(tapDrawColor:)];
...
- (void)setupClipMenu
{
    ...
    self.gridLayer = [[GTGridLayar alloc] init];
    ...
    [_rotateRatioBtn setBackgroundImage:GetImageWithName(@"gt_btn_rotate") forState:UIControlStateNormal];
    ...
    for(NSDictionary *info in _configuration.clipRatios){
        ...
        GTClipRatio *ratio = [[GTClipRatio alloc] initWithValue1:val1 value2:val2];
        ratio.titleFormat = info[@"titleFormat"];
        
        ratio.isLandscape = val1 > val2;
        
        GTClipItem *item = [[GTClipItem alloc] initWithFrame:CGRectMake(x, 0, W, H) image:iconImage target:self action:@selector(tapRadio:)];
        item.ratio = ratio;
        [item refreshViews];
...
- (void)clipBtn_click
{
    [self switchTool:GTImageEditTypeClip];
}
...
{
    for (GTClipItem *item in _clipMenu.subviews){
        if([item isKindOfClass:[GTClipItem class]]){
...
{
    [self switchTool:GTImageEditTypeFilter];
}

- (void)drawBtn_click
{
    [self switchTool:GTImageEditTypeDraw];
}

- (void)switchTool:(GTImageEditType)type
{
    ...
    switch (_selectToolType) {
        case GTImageEditTypeClip:
            [self setupClipMenu];
            break;
        case GTImageEditTypeFilter:
            [self setupFilterMenu];
            break;
        case GTImageEditTypeDraw:
            [self setupDrawMenu];
            break;
        default:
            break;
    ...
    if (_selectToolType & GTImageEditTypeFilter) {
    ...
    } else if (_selectToolType & GTImageEditTypeDraw) {
        _filterMenu.hidden = YES;
        _drawMenu.hidden = NO;
        _clipMenu.hidden = YES;
        _rotateRatioBtn.superview.hidden = YES;
    ...
    } else if (_selectToolType & GTImageEditTypeClip) {
        _filterMenu.hidden = YES;
        _drawMenu.hidden = YES;
        _clipMenu.hidden = NO;
...
{
    GTFilterItem *item = (GTFilterItem *)sender.view;
    if (self.selectFilterItem == item) return;
    
    self.selectFilterItem = item;
    
    //加滤镜
    self.imageView.image = [GTFilterTool filterImage:_editImage filterType:item.filterType];
}

- (void)setSelectFilterItem:(GTFilterItem *)selectFilterItem
{
...
{
    GTDrawItem *item = (GTDrawItem *)sender.view;
    if (self.selectDrawItem == item) return;
    
    self.selectDrawItem = item;
}

- (void)setSelectDrawItem:(GTDrawItem *)selectDrawItem
{
...
{
    GTClipItem *item = (GTClipItem *)sender.view;
    
    self.selectClipItem = item;
}

- (void)setSelectClipItem:(GTClipItem *)selectClipItem
{
...
- (void)setSelectClipRatio:(GTClipRatio *)selectClipRatio
{
