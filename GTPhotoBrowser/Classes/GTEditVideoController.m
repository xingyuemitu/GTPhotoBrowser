...
///////-----cell
@interface GTEditVideoCell : UICollectionViewCell

@property (nonatomic, strong) UIImageView *imageView;

@end

@implementation GTEditVideoCell
...
@protocol GTEditFrameViewDelegate <NSObject>
...
///////-----编辑框
@interface GTEditFrameView : UIView
{
...
@property (nonatomic, weak) id<GTEditFrameViewDelegate> delegate;

@end

@implementation GTEditFrameView
...
- (void)setupUI
{
    ...
    _leftView = [[UIImageView alloc] initWithImage:GetImageWithName(@"gt_ic_left")];
    ...
    _rightView = [[UIImageView alloc] initWithImage:GetImageWithName(@"gt_ic_right")];
...
///////-----editvc
@interface GTEditVideoController () <UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, GTEditFrameViewDelegate>
{
    UIView *_bottomView;
    UIButton *_cancelBtn;
    UIButton *_doneBtn;
...
@property (nonatomic, strong) GTEditFrameView *editView;
@property (nonatomic, strong) AVAssetImageGenerator *generator;

@end

@implementation GTEditVideoController
...
- (void)viewDidLayoutSubviews
{
    ...
    _cancelBtn.frame = CGRectMake(10+inset.left, 7, GetMatchValue(GetLocalLanguageTextValue(GTPhotoBrowserCancelText), 15, YES, bottomBtnH), bottomBtnH);
...
- (void)setupUI
{
    ...
    [self.collectionView registerClass:GTEditVideoCell.class forCellWithReuseIdentifier:@"GTEditVideoCell"];
    
    [self.view addSubview:self.collectionView];
    
    [self creatBottomView];
    
    self.editView = [[GTEditFrameView alloc] init];
...
- (void)creatBottomView
{
    ...
    [_cancelBtn setTitle:GetLocalLanguageTextValue(GTPhotoBrowserCancelText) forState:UIControlStateNormal];
    ...
    [_doneBtn setTitle:GetLocalLanguageTextValue(GTPhotoBrowserDoneText) forState:UIControlStateNormal];
...
- (void)btnDone_click
{
    ...
    [GTPhotoManager exportEditVideoForAsset:_avAsset range:[self getTimeRange] type:nav.configuration.exportVideoType complete:^(BOOL isSuc, PHAsset *asset) {
        [hud hide];
        if (isSuc) {
...
{
    GTEditVideoCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"GTEditVideoCell" forIndexPath:indexPath];
...
static const char _GTOperationCellKey;
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath
{
        ...
        if (!error && cgImg) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (row == nowIndexPath.row) {
                    [(GTEditVideoCell *)cell imageView].image = image;
                } else {
                    UIImage *cacheImage = _imageCache[@(nowIndexPath.row).stringValue];
                    if (cacheImage) {
                        [(GTEditVideoCell *)cell imageView].image = cacheImage;
                    }
                }
            });
    ...
    objc_setAssociatedObject(cell, &_GTOperationCellKey, op, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath
{
    NSBlockOperation *op = objc_getAssociatedObject(cell, &_GTOperationCellKey);
    if (op) {
        [op cancel];
        objc_removeAssociatedObjects(cell);
